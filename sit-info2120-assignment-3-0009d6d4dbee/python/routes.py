...
@app.route('/hash_all_passwords')
def hash_all_passwords():
    if (database.are_passwords_hashed()):
        flash('Passwords have already been hashed. No action taken.')
    else:
        database.hash_passwords()
        flash('Hashing complete.')
    return redirect(url_for('index'))
